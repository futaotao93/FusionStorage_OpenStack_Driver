...
class RestCommon(object):
    ...
    def call(self, url, method, data=None,
        ...
        try:
            ...
            LOG.exception(_('Bad response from server: %(url)s.'
        ...
        if not filter_flag:
            ...
            Response Data:{},
            Result Data:{}'''.format(
                call_url, method, data, result, result.json()))
    ...
    def query_all_volumes(self, pool_id, vol_name=None,
        ...
        params_filter = {}
        if vol_name:
            params_filter["volumeName"] = vol_name
        params["filters"] = params_filter
    ...
    def query_snapshot_by_volume(self, vol_name, batch_limit=1000,
                                 batch_num=1, snapshot_name=None):
        url = '/volume/snapshot/list'
        filters = {}
        if snapshot_name:
            filters["volumeName"] = snapshot_name
        params = {"volName": vol_name, "batchLimit": batch_limit,
                  "batchNum": batch_num, "filters": filters}
        result = self.call(url, "POST", params)
        self._assert_rest_result(
            result, _('Query snapshot by volume session error.'))
        return result.get("snapshotList")

    def query_snapshot_list(self, pool_id, page_num=1,
                            page_size=1000, snapshot_name=None):
        url = '/snapshot/list'
        params = {"poolId": pool_id, "pageNum": page_num,
                  "pageSize": page_size}
        params_filter = {}
        if snapshot_name:
            params_filter["volumeName"] = snapshot_name
        params["filters"] = params_filter
